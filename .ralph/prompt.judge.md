...
- `ralph` is a Go CLI with review convergence mode. Core packages: `config`, `runner`, `git`, `condition`, `prd`, `init`.
- Risk lenses:
  - Config schema drift: findings about `schemas/config.schema.json` vs `internal/config` type mismatch are high-severity.
  - Runner/condition semantics: `success()`, `failure()`, `always()`, `changed()`, `on_fail` must stay consistent.
  - Typed error contracts: `Code() string` and `ExitCode() int` must both be implemented.
...
1. Treat `Finding-Key` as the primary identity for each finding.
2. Build the set of finding keys in the latest review.
3. Compare with prior rounds and detect genuinely new finding keys.
4. Set `new_findings` to the number of new keys.
5. Set `new_finding_keys` to those keys (deduplicated, stable order).
6. Set `signal`:
   - `<promise>COMPLETE</promise>` only when `new_findings` is `0`.
   - otherwise use a non-complete value such as `CONTINUE`.
7. Recompute all fields from scratch each round; do not reuse prior JSON output.

If a review omits explicit `Finding-Key`, derive a stable fallback key from `file:line:summary-slug`.
...
- Use `<promise>COMPLETE</promise>` only when `new_findings` is `0`; otherwise use a non-complete value.
