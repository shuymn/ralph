...
- Add repository/domain-specific context here.
- Example:
  - Critical modules or directories to prioritize.
  - Invariants that must never break.
  - Known migration boundaries or out-of-scope areas.
...
7. Review against the resolved plan scope and DoD.
...
- Test coverage gaps
- Language best practices (idiomatic usage, naming, cohesion, error handling)
...
- Keep a strict `one root cause -> one finding` policy.
- Do not split one root cause into multiple findings with different wording.
- Do not merge unrelated root causes into one finding.
...
- For each finding, include:
  - `Finding-Key`: required stable identifier with this format:
    - `anchor_file:anchor_line:scope_sig:summary_slug`
    - `anchor_file`: normalized repository-relative path using `/` separators.
    - `anchor_line`: integer start line (`>= 0`).
    - `scope_sig`: first 8 hex chars of `sha256` over sorted unique related file paths joined by `\n`.
    - `summary_slug`: lowercase slug (`[a-z0-9-]`, max 48 chars, collapse repeated `-`).
  - `File`: required anchor location in one of these forms only:
    - `path#L<start>`
    - `path#L<start>-L<end>`
  - `Related-Files`: required when the finding spans multiple files.
    - Comma-separated repository-relative paths.
...
- Before finalizing output, run a deterministic self-check:
  - Every finding has all required fields and label names exactly as specified.
  - Every `Finding-Key` is unique within the report.
  - `File` format matches the required grammar.
  - `Fix-Points` matches `Fix-Effort`.
  - `Fix-Total-Points` equals the sum of all `Fix-Points`.
